...
if __name__ == "__main__":
    ...
    param_set.add_parameters(lr=[0.001, 0.01, 0.0001])
    param_set.add_parameters(init_fmaps=[8])
    param_set.add_parameters(zoom_level=[0, 1])
    param_set.add_parameters(loss=["both", "dice", "bce"])
    param_set.add_parameters(sparse_start_after=[-1, 0, 1, 5, 10, 20])
    param_set.add_parameters(aug_hed_bias_range=[0.001])
    param_set.add_parameters(aug_hed_coef_range=[0.001])
