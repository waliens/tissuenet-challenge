...
def make_experiment(env_params, segpc_rr, exp_type="self-train"):
    if exp_type not in {"self-train", "baseline-noself"}:
        raise ValueError("incorrect exp type '{}'".format(exp_type))

    exp_name = "segpc-{}-rr-{:0.4f}".format(exp_type, segpc_rr)
    ...
    param_set.add_parameters(segpc_rr=segpc_rr)
    ...
    for exp_type in ["self-train", "baseline-noself"]:
        for segpc_rr in [1.0, 0.95, 0.85, 0.8, 0.75, 0.60, 0.5, 0.25]:
            if segpc_rr > 0.9999 and exp_type == "baseline-noself":
                continue
            print("Exp '{}' with segpc_rr={}".format(exp_type, segpc_rr))
            experiment = make_experiment(env_params, segpc_rr, exp_type=exp_type)
